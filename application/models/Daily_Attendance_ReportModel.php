...
class Daily_Attendance_ReportModel extends CI_Model
{
    ...
    public function select_emp_data($data)
    {
        if ($data['emp_id'] == 0) {
            ...
            $select_query = $this->db->query("SELECT fp_timestamp_id,employee.employee_id,concat(first_name,' ',middle_name,' ',last_name) as emp_name,type,designation,agreement_type,phone_no,timestamp_date,timestamp_time,is_check_in,fc.start_time,fc.end_time from employee inner join fp_timestamp on employee.employee_id=fp_timestamp.employee_id  inner join fp_employee_category fec on fec.employee_id=employee.employee_id  inner join fp_category fc on fc.fp_category_id=fec.fp_category_id where fec.status=1 AND timestamp_date='$emp_year-$emp_month-$emp_day' order by timestamp_date,timestamp_time");
        } else {
            $emp_id = $data['emp_id'];
            $emp_year = $data['emp_year'];
            $emp_month = $data['emp_month'];
